...
db = database  # Alias for backward compatibility
...
async def get_channels() -> List[int]:
    try:
        channels = await channels_collection.find({"status": "active"}).to_list(length=None)
...
async def channel_exist(channel_id: int) -> bool:
    return bool(await fsub_data.find_one({'_id': channel_id}))
...
async def set_channel_mode(channel_id: int, mode: str) -> None:
    await fsub_data.update_one({'_id': channel_id}, {'$set': {'mode': mode}}, upsert=True)
...
async def del_req_user(channel_id: int, user_id: int) -> None:
    await rqst_fsub_Channel_data.update_one({'_id': channel_id}, {'$pull': {'user_ids': user_id}})

async def req_user_exist(channel_id: int, user_id: int) -> bool:
    try:
        found = await rqst_fsub_Channel_data.find_one({'_id': channel_id, 'user_ids': user_id})
        return bool(found)
    except Exception as e:
...
async def add_rqst_channel(channel_id: int) -> None:
    try:
        await rqst_fsub_Channel_data.update_one({'_id': channel_id}, {'$setOnInsert': {'user_ids': []}}, upsert=True)
    except Exception as e:
...
async def get_all_req_users(channel_id: int) -> List[int]:
    try:
        doc = await rqst_fsub_Channel_data.find_one({'_id': channel_id})
...
async def del_all_req_users(channel_id: int) -> None:
    try:
        await rqst_fsub_Channel_data.update_one({'_id': channel_id}, {'$set': {'user_ids': []}}, upsert=True)
    except Exception as e:
